...
class LoyaltyPointsService {
    ...
    calculatePoints(services) {
        ...
        const points = this.POINTS_RULES[service] || 10;
        return { points, category: 'basic_service' };
    }
    /**
     * Award points to a user after booking completion
     */
    async awardPoints(userId, bookingId, services) {
     ...
     */
    async redeemPoints(userId, bookingId) {
        const supabase = this.getSupabase();
        try {
            // Get current user points
            const { data: user, error: userError } = await supabase
...
//# sourceMappingURL=LoyaltyPointsService.js.map