...
const LoyaltyPointsService_1 = require("../services/LoyaltyPointsService");
const router = express_1.default.Router();
const bookingService = new BookingService_1.BookingService();
const authService = new AuthService_1.AuthService();
const emailService = new EmailService_1.EmailService();
const loyaltyService = new LoyaltyPointsService_1.LoyaltyPointsService();
...
router.put('/bookings/:bookingId/status', verifyAdminToken, async (req, res) => {
    ...
    try {
        ...
        if (status === 'completed' && booking.user_id) {
            try {
                const services = booking.service.split(',').map((s) => s.trim());
